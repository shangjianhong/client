...
public class BatchUserStepProcess implements Protocalable {
	private static Logger logger = LoggerFactory.getLogger(BatchUserStepProcess.class);
	...
	public ResponseData procssRequest(RequestData reqd) throws Exception {
		...
		try {
			...
			String batchUserStep = (String)dataSet.get(UserStep.KEY_BATCH_USER_STEP);
			if(batchUserStep!=null&&!"".equals(batchUserStep)){
				String[] userStepArray = batchUserStep.split(UserStep.BATCH_DELIMITER);
				if(userStepArray!=null&&userStepArray.length>0){
					for(int i =0;i<userStepArray.length;i++){
						JSONObject userStepJson = (JSONObject) JSONValue.parse(userStepArray[i]);
						
						String registerId = (String)userStepJson.get(RegisterUser.KEY_REGISTER_ID);
						String password = (String)userStepJson.get(RegisterUser.KEY_PASSWORD);
						long adId = (Long)userStepJson.get(AdInfo.KEY_AD_ID);
						long step = (Long)userStepJson.get(UserStep.KEY_USER_STEP);
						long reportDate = (Long)userStepJson.get(UserStep.KEY_REPORT_DATE);
